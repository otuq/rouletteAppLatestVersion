...
struct Save {
    ...
    func saveRouletteData( title: String, flag: Bool, colorIndex: Int ) {
        ...
        // 新規データの作成
        if homeVC.newDataBTNPresent.isSelected {
            ...
            print("データを新規作成しました。")
            // データセットの更新
        } else if homeVC.setDataBTNPresent.isSelected {
    ...
    private func setAndDismiss(_ homeVC: HomeViewController) {
        if homeVC.dataSet == nil {
            homeVC.startBTNPresent.titleLabel?.removeFromSuperview()
            homeVC.view.addSubview(homeVC.startLabel)
        }
        homeVC.dataSet = dataSet
        homeVC.titleLBPresent.text = dataSet.title.isEmpty ? "No title": dataSet.title
        homeVC.newDataBTNPresent.isSelected = false
        homeVC.setDataBTNPresent.isSelected = false
