...
class SaveRoulette {
    ...
    func saveRouletteData( title: String, flag: Bool, colorIndex: Int ) {
        ...
        if homeVC.newDataButton.isSelected{
            ...
            let titleA = "ウィンドウを閉じる", titleB = "保存してセット"
            ...
            let titleA = "ウィンドウを閉じる", titleB = "上書きをする"
            alertVC.selectTwoChoice(titleA: titleA, titleB: titleB) {
                try! self.realm.write({
                    self.dataSet.list.removeAll()
                })
    ...
    func closeWindow() {
        ...
        let titleA = "ウィンドウを閉じる", titleB = "編集を続ける"
        alertController.selectTwoChoice(titleA: titleA, titleB: titleB) {
